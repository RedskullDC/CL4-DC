...
bool	_issetbuf;
bool	_started;
bool	bp_flag;		// broken pipe interrupt has occurred.
int		sigSeen_0;
bool	term_saved = false;	// Tty_Construct()
...
short	dscreen(BTAB *btptr, int ScreenNo, short CurrPTno);
...
bool	samekey(int TDno, char *a2, int TDno2, char *a4);
...
char		*_cgiGetPartValue(char *s2, int);
...
void	scat(short Attr, bool on_off, int FieldType, short a4, bool a5);
...
int		gettdfno(ENTAB *entab, int *TTno, char *FLDtype, bool UpdateTT);
...
void	prdebcon(int LineNo, bool Condition, int a3);
...
char	*convstr(char *Buffer, bool encode);
bool	evaliex(ENTAB *entab, bool Skip);
...
bool	_tdopen(int DBno, TDesc *TTptr);
...
int		egetkey(BTAB *bt, PTAB *ptb, XTAB *xtb, FLDdesc *fld, int TDno, int RecNo, short CurrPTno);
...
void	__getrnge(int *a1, int *a2, char *a3);
...
// DC extension
bool	popenlib(int Entno);
void	closelibs(void);
bool	plibcall(int ENTno);
