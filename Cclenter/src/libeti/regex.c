...
char * libform_regex(char *addrc, char *addrl, char *a1)
{
	intptr_t cur, in;
	intptr_t *adx;
	char *p1, *p2;
...
intptr_t __execute(char *addrc, char *addrl)
{
	char *p1, *p2, c;
	intptr_t i;
...
intptr_t __advance(char *alp, char *aep)
{
	char *lp, *ep, *curlp;
	char *sep, *dp;
...
intptr_t __cclass(char *aset, char ac, intptr_t af)
{
	char *set, c;
	intptr_t n;
...
intptr_t __xpush(intptr_t i, char *p)
{
	if (__lptr_ >= __eptr_) {
...
intptr_t __getrnge(intptr_t *i, intptr_t *j, char *k)
{
	*i = (*k++&0377);
	if (*k == (char)-1)
		*j = 20000;
