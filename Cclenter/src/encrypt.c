...
static void byteReverse OF((LONG *buffer, int byteCount));  
...
char *sha0_encrypt(char *src, char *s)
{
	SHS_INFO shsInfo;

	shsInit(&shsInfo);
    shsUpdate(&shsInfo, src, strlen(src));
    shsFinal(&shsInfo);
    sprintf(s, "%08lx%08lx%08lx%08lx%08lx", shsInfo.digest[0], shsInfo.digest[1], shsInfo.digest[2], shsInfo.digest[3], shsInfo.digest[4]);
    return s;
}