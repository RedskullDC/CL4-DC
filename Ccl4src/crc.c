...
unsigned short acrc(unsigned short CRC, char *buff, unsigned int len)
{
    ...
    //for ( i = 1; i <= len; ++i )
	while (len--)
		CRC = crc16t_[(unsigned char)(CRC ^ *buff++)] ^ (CRC >> 8);
...
unsigned short pcrc(unsigned short CRC, TDef *tdef, int TDFsize, short TDFtype, unsigned int a5)
{
     ...
     v5 = (char *)tdef;
     buff = (char *)mmalloc(a5 * TDFsize);
     v10 = 0;
     for ( i = buff; v10 < a5; v5 += TDFsize )
     {
          xtodom(i, TDFsize, TDFtype, v5);
          v10++;
          i += TDFsize;
     }
     CRC = acrc(CRC, buff, TDFsize * a5);
     mfree_0(buff);
     return CRC;
}
